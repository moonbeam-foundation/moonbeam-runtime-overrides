use {
	clap::Parser,
	std::path::PathBuf,
	toml_edit::{Document, Item, Value, Formatted, Array},
};
...
fn main() {
	...
	let mut indices_to_remove: Vec<_> = members.iter().enumerate().filter_map(|(i, member)| {
    ...
    let shared = ["primitives/rpc/evm-tracing-events", "runtime/evm_tracer", "primitives/rpc/debug", "primitives/ext"];
    let ignore_git = ["runtime/moonbeam", "runtime/moonriver", "runtime/moonbase", "runtime/common"];
